...
- Ведущие нули: ИНН может начинаться с нуля. Если использовать числовой тип данных (INTEGER, BIGINT и т.п.), ведущие нули будут автоматически отбрасываться. Например, ИНН "0123456789" при хранении в числовом формате станет "123456789", что нарушит его корректность.
- ИНН — это не число в математическом смысле: ИНН используется для идентификации и не участвует в арифметических операциях (сложение, умножение и т.п.). Хранение его как строки позволяет избежать потенциальных ошибок и недоразумений, связанных с его интерпретацией как числа.
...
+ ИНН может меняться, что недопустимо для первичного ключа, который должен быть стабильным. Хранение длинного строкового значения как первичного ключа может отрицательно сказаться на производительности базы данных.
